...
ua = 'com.ss.android.ugc.aweme.lite/250101 (Linux; U; Android 10; zh_CN; Redmi Note 7; Build/QKQ1.190910.002;tt-ok/3.12.13.1)'
debug_mode = 0
...
class User():
	...
	def find_params(self):
		...
		self.shopping_completed = shopping_info.get('completed')
		...
		try:
			for count in range(water_count):
				...
				if res.get('status_code') == 0:
					...
					if shake_tree_state == 2 or shake_tree_state == 3:
	...
	# 待完善
	def game_orchard_three_gift_reward(self):
		time.sleep(random.uniform(3, 5))
		# task_id 0 早
		# task_id 1 中
		# task_id 2 晚
		try:
			task_id = None
			url = f'https://minigame.zijieapi.com/ttgame/game_orchard/three_gift/reward?task_id={task_id}&watch_ad=1&extra_ad_num=0&' + self.clear_mode_params
			res = self.struct_requests(method='get', url=url, payload=None, referer=referer)
			if res.get('status_code') == 0:
				reward_num = res.get('data').get('reward_item').get('num')
				reward_name = res.get('data').get('reward_item').get('name')
				...
				cur_fertilizer_lite = res.get('data').get('fertilizer').get('lite')
				cur_fertilizer_normal = res.get('data').get('fertilizer').get('normal')
				print(f'【果园 三餐礼包】获得{reward_num}个{reward_name}')
				print(f'当前水滴{cur_water_num},小化肥{cur_fertilizer_lite},大化肥{cur_fertilizer_normal}')
			else:
	...
	def game_orchard_run(self):
		bottle_state = self.game_orchard_polling_info_write()[1]
		...
		# 查询
		water_num = self.game_orchard_polling_info_write()[0]
		# 剩余水 其余全部浇水
